...
2. **Read the spec end-to-end.** Note every entity, endpoint, migration and test it proposes.
    ...
    **Check the spec is written, not templated.** A section whose body is still the template's — "(Filled in at the end.)", a `{…}` placeholder, "TBD", an "N/A" — is a **High** finding: the Final Compliance Report in particular is the spec's own evidence that the host's rules were checked, and an unfilled one means they were not. A section the spec does not need is removed, not left blank. **Check the size**: the TLDR names the spec `bounded` or `full` (`../spec-writing/SKILL.md` step 3); a missing size is **High**, a bounded spec carrying sections outside its six is **High**, and a spec several times longer than the change it describes is **High** whatever its size says. **Check for an argued-around host skill**: a spec that explains why it departs from a host skill's steps, rather than proposing the one-line fix to that `SKILL.md` in its first phase, is **High**.

3. **Spawn the audit agents** — see **Parallel Audit Strategy** below. A **full** spec gets the four in parallel. A **bounded** spec gets **one** agent, doing Agent 1's and Agent 4's tasks together: a change of that size has no contract surface for Agent 2 and no migration or auth surface for Agent 3, and four agents reading a forty-line change is the audit costing more than the work. Each agent receives: the full spec text, the list of files and modules it references, and the host's root `AGENTS.md` / `CLAUDE.md` plus the nearest one to each area touched. **Never build anything to audit the spec** — no scratch implementation, no trial run of a phase.
4. **Synthesise**: merge the agents' outputs into the Readiness Report. Where agents contradict, trust the one with more specific `file:line` evidence.
