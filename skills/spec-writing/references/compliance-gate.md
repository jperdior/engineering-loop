...
| Gates | Is there a `## Gates` section whose every `- `command`` line is a validation command the host's own docs name, in the docs' order, with `_Cleanup:_`, `_Excludes:_` and `_Denials:_` present only where the docs give a basis? | Yes. `<loop>/parse-ledger.sh <spec> --gates` exits 0. A gate the docs do not name, or a documented gate left out, is High. Each phase ends with those gates green and the app in a working state. |
